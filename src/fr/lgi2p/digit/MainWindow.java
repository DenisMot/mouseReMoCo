...
public final class MainWindow implements  MouseMotionListener, MouseListener, KeyListener, ActionListener {
	...
	public void buildAndShow() {
		...
		//frame.setUndecorated(configuration.getFrameUndecorated());	// more screen, but less flexibility to move the widow
	...
	private void DoCycleChange() {
		...
		if  (!this.isCycling & NbRecDone < configuration.getCycleMaxNumber() && NbRestDone < configuration.getCycleMaxNumber() ) {
		...
		String Message  = " RecordDone=" + NbRecDone + " PauseDone=" + NbRestDone + " ToDo=" + configuration.getCycleMaxNumber() ;
		boolean isSequenceDone = (NbRecDone >= configuration.getCycleMaxNumber() && NbRestDone >= configuration.getCycleMaxNumber()) ;
		...
		if (!isSequenceDone){ 
			if (NbRecDone <= NbRestDone) { 
				...
				outputMouse.writeNumericMarker(PAUSE_MARKER);		// marker for sync with external devices reading only integers
				outputMouse.writeData(0L, 0, 0, false);			// show the end of a record with an "all zero line" to ease parsing the output file
	...
	private void mouseMovedOrDragged(MouseEvent mouseEvent){
		...
		if ( this.recording ) {
			...
			//if (true) {
				...
				if (performanceAtTask != null)  performanceAtTask.mouseMoved(T, X, Y, isInside, d);
			}
		}
	}
	...
	public void keyPressed(KeyEvent keyEvent) {		
		// key pressed depends on keyboard and locale... not funny for the user to type q for z
	}
	...
	public void keyTyped(KeyEvent keyEvent) {
		...
		case 'w':
			...
			actionPerformed( new ActionEvent( this , ActionEvent.ACTION_PERFORMED , "DoToggleDisplayEffectiveTolerance") ); 
